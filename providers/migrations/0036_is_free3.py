    ...
    for provider in Provider.objects.all():
        ...
        if not len(services) == 0:
            ...
            for service in services:
                if (
                    service.full_day_price is not None and service.full_day_price > 0
                ) or (service.hourly_price is not None and service.hourly_price > 0):
                    provider.is_volunteer_organization = False
                elif service.full_day_price is None and service.hourly_price is None:
                    provider.is_volunteer_organization = False
            provider.save()
...
class Migration(migrations.Migration):
    dependencies = [
        ('providers', '0035_alter_provider_service_description'),
    ]
